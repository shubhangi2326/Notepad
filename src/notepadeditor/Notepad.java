...
public class Notepad implements ActionListener{
    ...
    JMenuItem neww ,save, saveas,open, exit , page_setup,print  ;
    ...
    public Notepad()
    {
        ...
        Image icon= Toolkit.getDefaultToolkit().getImage("C:\\Users\\Mahaj\\Documents\\NetBeansProjects\\NotepadEditor\\src\\notepadeditor\\notepad-icon.png");
       ...
       page_setup = new JMenuItem("Page Steup");
...
else if(i==1)
{
  textarea.setText("");
   ...
   public void save() {
    // If the filee is null (i.e., no file has been saved yet), prompt the user to save the file
    if (filee == null) {
        saveAs();  // Prompt the user to select a location for saving
    } else {
        try {
            // Proceed with saving the content to the existing filee
            String text = textarea.getText();
            byte[] b = text.getBytes();
            FileOutputStream fos = new FileOutputStream(filee);
            fos.write(b);
            fos.close();
            JOptionPane.showMessageDialog(jf, "File saved successfully!", "Save", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
...
fos.close();
 ...
 public  void replace()
 {
  ...
  {
      Color d=JColorChooser.showDialog(jf, "Select Textarea Color" , Color.white);
      textarea.setBackground(d);
  }
public void  openFontFrame()
{
  ...
  cb_font_family.setBounds(50, 50, 200, 40);
   ...
   String[] font_style={"Pain","Bold","Italic"};
